...
public class MyInfoEditTester extends InstrumentationTestCase {
     ...
     //   Runtime.getRuntime().exec(new String[]{"am", "force-stop", "MSD"});
      ...
      //          .scrollable(true));
       // appViews.swipeLeft(1000);
        ...
        device.wait(Until.hasObject(By.text("Edit")), 8000);
        UiObject2 edit = device.findObject(By.text("Edit"));
        edit.click();

        device.wait(Until.hasObject(By.text("EditProfile")), 5000);
        UiObject2 editBanner = device.findObject(By.text("EditProfile"));

        device.waitForIdle(3000);
        //  UiObject2 resultText = device.findObject(By.clazz("android.widget.EditText"));
        String result = editBanner.getText();
        assertTrue(result.equals("EditProfile"));
    }
