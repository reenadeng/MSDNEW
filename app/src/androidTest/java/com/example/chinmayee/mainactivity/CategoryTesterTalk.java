...
public class CategoryTesterTalk extends InstrumentationTestCase {
    ...
    public void setUp() throws Exception {
     ...
     //   Runtime.getRuntime().exec(new String[]{"am", "force-stop", "MSD"});
        ...
        UiObject menuBanner = device.findObject(new UiSelector().resourceId("com.example.chinmayee.mainactivity:id/talk"));
        menuBanner.click();


        device.wait(Until.hasObject(By.text("search result for : talk")), 5000);
        UiObject2 banner = device.findObject(By.text("search result for : talk"));
        device.waitForIdle(3000);
        //  UiObject2 resultText = device.findObject(By.clazz("android.widget.EditText"));
        String result = banner.getText();
        assertTrue(result.equals("search result for : talk"));
    }
